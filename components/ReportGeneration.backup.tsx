...
*分析师：AI智能分析系统*
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const newReportForAssessment: Report = {
        id: `report-${Date.now()}`,
        title: topic,
        content: reportText,
        summary: reportText.split('\n').slice(0, 3).join('\n'),
        articles: data.metadata?.dataSources || [],
        summaries: reportText.split('\n\n').map((section: string, index: number) => ({
          id: `summary-${index}`,
          content: section,
          source: 'AI生成',
        ...
        metadata: {
          ...
          word_count: reportText.length,
          source_count: data.metadata?.dataSourceCount || 0
        },
        quality: data.qualityAssessment || { rating: 0, feedback: '' }
      };
      setReportForAssessment(newReportForAssessment);
              ...
              {generatedReport && (
                <div className="flex gap-2">
                  <button
                    ...
                    className="fenghuo-button-secondary flex items-center gap-2"
                  >
                    <span>📥</span>
                    导出报告
                  </button>
              ...
              <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <span className="font-medium">{reportMetadata.wordCount || 0}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">数据源：</span>
                    <span className="font-medium">{reportMetadata.dataSourceCount || 0}个</span>
                  </div>
                  <div>
                    <span className="text-gray-600">生成时间：</span>
                    <span className="font-medium">{reportMetadata.processingTime || 0}秒</span>
                  </div>
                  <div>
                    <span className="text-gray-600">模板：</span>
                    <span className="font-medium">{reportMetadata.template || template}</span>
                  </div>
                </div>
              </div>
            )}
